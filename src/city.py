    ...
    def get_info(cls, dishe_name: str) -> Optional[Dict[str, Any]]:
        ...
        for dishe_type, dishes in DISHES.items():
            if dishe_name in dishes:
                return {"name": dishe_name, "type": dishe_type, **dishes[dishe_name]}
...
class Citizen(Entity):
    ...
    def what_to_prepare(self, context: Optional[Dict[str, Any]] = None) -> Dishe:
        ...
        return Dishe(
            name=dishe_info["name"], ingredients=dishe_info["required_ingredients"]
        )
    ...
    def buy_ingredients(self, dishe: Dishe) -> str:
        ...
        for ingredient_name in dishe.ingredients:
            ...
            ingredient_paths = [
                ingredient
                for ingredient in ingredient_paths
                if not any(word in ingredient.lower() for word in ["seed", "ripe"])
            ]

            if not ingredient_paths:
                write_json(
                    {"who.want": ingredient_name}, WISH_DIR, f"{ingredient_name}.wish"
                )
                continue
            ...
            for ingredient_path in ingredient_paths:
                ...
                if ingredient_name in ingredient_path:
                    ...
                    write_json(
                        ingredient,
                        os.path.join(CITY_DIR, self.name, "ingredients"),
                        filename,
                    )
                    os.remove(ingredient_path)
                    ingredient_paths.remove(ingredient_path)
                    self._add_event(
                        event_type=f"{ingredient.seed.name}.bought.from.{ingredient.seed.farmer}",
                        data=ingredient,
                    )
                    break
    ...
    def cook(self, dishe: Dishe) -> None:
        ...
        for ingredient_name in dishe.ingredients:
            my_ingredient_patterns = [
                os.path.join(CITY_DIR, self.name, f"{ingredient_name}.*")
            ]
            my_ingredient_paths = get_files_path_by_patterns(
                patterns=my_ingredient_patterns
            )
